		...
		<scroll-view scroll-y @scrolltolower="scrollBottom" class="content">
			<view class="list" v-if="list.length > 0">
				<view class="item flex" v-for="item in list" :key="item.id"
					@click="$utils.toPage(`/pages/mine/messages/detail?id=${item.id}`)">
					...
					<view class="op">
						<view class="status error" v-if="item.status == 0"><u-badge :isDot="true" type="error"
								class="u-badge"></u-badge> 未读</view>
						<view class="status success" v-if="item.status == 1"><u-badge :isDot="true" type="success"
								class="u-badge"></u-badge> 已读</view>
					</view>
				</view>
			</view>
			<view class="m-noData" v-else>
				<view class="item">
					<view class="icon">
						<image src="@/static/new/nodata.png" mode="widthFix"></image>
					</view>
