<template>
	<view class="container" style="background:#c8131e url(/static/new/bg.jpg) no-repeat center top / 100%;">
		<headerView>
			...
			<view>{{settingInfo.name}}打卡中心</view>
		</headerView>
		<view class="content">
			<view class="main">
				<view class="tips">
				...
				<view class="progress">
					...
					<view class="num" v-if="signInfo.sign_people == 0">0%</view>
					<view class="num" v-else>{{percentValue}}%</view>
				</view>
				<view class="title">
					下档奖励升级: <text class="red">{{signInfo.min_price}}~{{signInfo.max_price}}</text> 元 <br>
					今日收入: <text class="red">{{signInfo.sign_dividend}}+{{signInfo.sign_price}}</text> 元
				</view> -->
				<view class="btn_1">
				...
				<view class="jf">
					<view class="item" :class="signInfo.sign_num >= item.day?'on':''" v-for="(item,index) in signInfo.sign_points"><u-icon name="checkmark-circle-fill"></u-icon>
						<text>{{item.day}}天→ {{item.points}}积分</text>
					</view>
				</view>
			</view>
			<view class="rule">
				<view class="rule_content" v-html="signInfo.sign_rule"></view>
			</view>
			<!-- <view class="card">
				<view class="title">
					<view class="tit">
		...
		<view class="f-btn">
			<view class="item" @click="onSup()">
				<image src="/static/new/ca1.png" mode="heightFix"></image> <text>补签</text>
			</view>
			<view class="item" @click="isBtn?onDayClick():''">
				<image src="/static/new/ca2.png" mode="heightFix"></image> <text>{{isBtn?'签到':'今日已签到'}}</text>
			</view>
		</view>
				...
				<view class="picker-area-bd">
					<picker-view class="picker-view" indicator-class="picker-view-selected" :value="dateList.selectIndex" @change="bindChange">
						...
						<picker-view-column v-if="dateList.calendar.length > 0">
							<view class="picker-item" v-for="(item,index) in dateList.calendar[dateList.selectIndex[0]].list" :key="index">
								{{item.month}}月
							</view>
						</picker-view-column>
					</picker-view>
		...
		<u-popup :show="qdShow" mode="center" round="32" :closeable="true" @close="qdShow=false">
			<view class="van-dialog van-qd">
				<view class="van-dialog__header">签到提示</view>
		...
		methods: {
			...
			getSignInRecord() {
				...
				signInfoAPI({
					...
					this.isBtn = this.signInfo.today_sign
					this.percentValue = Math.floor(Number(this.signInfo.sign_people) / Number(this.signInfo
						.sign_upgrade_people) * 100);
					this.percentValue = Math.max(0, Math.min(100, Number(this.percentValue)));
			...
			getSup() {
				supplementaryAPI({
					...
					console.log(this.supList+"shuliang")
					this.dayIndex = 0;
		...
		.main {
			...
			.jf {
				...
				.item {
					min-width: 300rpx;
					flex: 1;
					color: rgba(4, 10, 19, .4);
					display: flex;
					align-items: center;
					::v-deep .u-icon__icon {
						color: rgba(4, 10, 19, .4) !important;
					}
					text {
						padding-left: 10rpx;
					}
				...
				.on {
					color: rgba(246, 42, 45, 1);
					::v-deep .u-icon__icon {
						color: rgba(246, 42, 45, 1) !important;
					}
				}
			}
		}
