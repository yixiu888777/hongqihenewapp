...
<template>
	<view class="container" style="background-color: rgba(186, 28, 29, 1);" v-if="isPageShow">
		...
		<view class="main">
			...
			<view class="video">
				<!-- APP 端原生 video 层级高于弹窗，弹窗打开时用占位图卸载 video -->
				<video id="myVideo" style="width: 100%; height: 480rpx;" loop :show-center-play-btn="true"
					...
					:autoplay="videoShow" controls @pause="onPause" v-if="showHomeVideo"></video>
			...
			<!-- <view class="zxrk" @click="$utils.toPage('/pages/index/newsList')">更多资讯入口</view>
			 --><view class="tab1">
				<view class="item1"  @click="$utils.toPage('/pages/index/newsList')">
					<image src="/static/shoye/1.png" mode="widthFix"></image>
				
					
				</view>
				<view class="item1" @click="dengdaikaifang()">
					<image src="/static/shoye/2.png" mode="widthFix"></image>
				
					
				</view>
				<view class="item1" @click="$utils.toPage('/pages/index/jiyuanjijin')">
					<image src="/static/shoye/3.png" mode="widthFix"></image>
				
					
				</view>
				<view class="item1" @click="$utils.toPage('/pages/index/downloadApp')">
					<image src="/static/shoye/4.png" mode="widthFix"></image>
	...
	<!-- 		<view class="titlex">
	 ...
	 -->	</view>
		...
		<u-popup :show="waitOpenShow" mode="center" round="32" :closeable="true" @close="onCloseWaitOpen">
			<view class="van-dialog van-dialog2">
				<view class="van-dialog__header">温馨提示</view>
				<view class="van-dialog__content">
					<view class="desc">正在备案，即将开放！</view>
				</view>
				<view class="van-dialog__footer">
					<view class="van-success" @tap="onCloseWaitOpen">关闭</view>
				</view>
			</view>
		</u-popup>
...
<script>
	...
	export default {
		...
		computed: {
			...
			showHomeVideo() {
				// #ifdef APP-PLUS
				return this.myVideoShow && !this.anyPopupOpen
				// #endif
				// #ifndef APP-PLUS
				return this.myVideoShow
				// #endif
			}
		},
		onLoad() {
		...
		methods: {
			...
			dengdaikaifang() {
				//this.waitOpenShow = true
				uni.navigateTo({
					url: '/pages/baoxian/baodanone'
				});
			},
			onCloseWaitOpen() {
				this.waitOpenShow = false
			},
			onjyShow() {
				this.jyShow = false;
				this.myVideoShow = true;
			...
			async loadData() {
				...
				const bannerlist = this.getCache('bannerlist');
				if (!bannerlist) {
					commonBannerApi({
						cate: 1
					}).then(res => {
						this.swiperList = res.data.data;
						this.setCache('bannerlist',this.servicesUnit,36000)
					})
				}else{
					this.swiperList = this.getCache('bannerlist');
				}
				// 滚动通知
				/* commonScrollApi({
					is_popup: 2
				...
				const Scrolllist = this.getCache('Scrolllist');
				if (!Scrolllist) {
					commonScrollApi({
						is_popup: 2
					}).then(res => {
						this.noticeText = res.data.data[0].content
						this.setCache('Scrolllist',this.noticeText,6000)
					})
				}else{
					this.noticeText = this.getCache('Scrolllist');
				}
				
				// 获取系统配置
				
				const settingInfo = this.getCache('settingInfoindex');
				if (!settingInfo) {
					getSettingApi().then(res => {
						this.settingInfo = res.data.data
						uni.setStorageSync('settingInfo', res.data.data)
						this.setCache('settingInfoindex',this.settingInfo,6000)
					})
				}else{
					this.settingInfo = this.getCache('settingInfoindex');
					uni.setStorageSync('settingInfo', this.getCache('settingInfoindex'))
				}
