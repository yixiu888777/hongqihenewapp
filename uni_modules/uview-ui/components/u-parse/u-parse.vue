...
<script>
	...
	export default {
		...
		methods: {
				...
				(function traversal(nodes) {
					for (let i = 0; i < nodes.length; i++) {
						...
						if (node.type == 'text')
							...
							// 块级标签前后加换行
							const isBlock = node.name == 'p' || node.name == 'div' || node.name == 'tr' || node.name ==
								'li' || (node.name[0] == 'h' && node.name[1] > '0' && node.name[1] < '7')
			...
			getRect() {
				return new Promise((resolve, reject) => {
					uni.createSelectorQuery()
						...
						.in(this)
						// #endif
						.select('#_root').boundingClientRect().exec(res => res[0] ? resolve(res[0]) : reject(
			 ...
			 */
			_set(nodes, append) {
				this.$refs.web.evalJs('setContent(' + JSON.stringify(nodes) + ',' + JSON.stringify([this.bgColor, this
					.errorImg, this.loadingImg, this.pauseVideo, this.scrollTable, this.selectable
				]) + ',' + append + ')')
			},
