<!-- 有用 -->
<template>
	<!-- <view class="container" :style="'background: url('+ settingInfo.background.home_bg_image +') no-repeat center top / cover;background-attachment: fixed;'"> -->
	<view class="container" style="background-color: rgba(186, 28, 29, 1);">
		...
		<view class="main">
			<swiper class="swiper" style="margin: 0;" circular :interval="3000" :duration="1000" indicator-dots
			...
			<view class="noticex">
				...
				<text>公告：</text>
				<text v-html="noticeText"></text>
			</view>
			<!-- <swiper class="swiper" circular :autoplay="!videoShow" :interval="3000" :duration="1000"> -->
			
				
						<!-- <video style="height: 280rpx;width: 690rpx;" id="myVideo" loop :show-center-play-btn="true" :src="item.file" :poster="item.image" object-fit="fill" :autoplay="isBo" :muted="isBo" controls></video> -->
						<!-- <nVideo :src="item.image" style="height: 280rpx;width: 690rpx;" v-if="isPaly"></nVideo>  @click.stop="isBo?stop():play()"-->
						<view class="video">
							<video id="myVideo" style="width: 100%;" loop :show-center-play-btn="true"
							:src="settingInfo.basic.index_video"
								object-fit="fill" :autoplay="videoShow" controls @pause="onPause"></video>
						</view>
			...
			<!-- <view class="notice" v-if="noticeText">
				<view class="icon">
					<image src="/static/new/notice.png" mode="aspectFit"></image>
				</view>
				<view class="text">
					<u-notice-bar :text="noticeText" icon="" speed="40" bgColor="rgba(243, 62, 49, 0)" color="#FF363E"
						fontSize="12"></u-notice-bar>
				</view>
			</view> -->
			<view class="zxrk" @click="$utils.toPage('/pages/index/newsList')">
				更多资讯入口
			</view>
			<view class="titlex">
				<!-- <text>{{settingInfo.background.index_ad_image}}</text> -->
				<text>百年复兴</text>
				<text>纪元基金</text>
			</view>
			<view class="tab1">
		...
		<u-popup :show="noticeShow" mode="center"  :overlayStyle="kkl" >
			<view class="notice_mask" >
				...
				<view style="display: flex;justify-content: space-between;margin-bottom: 30rpx;color: #fff;align-items: center;font-size: 30rpx;font-weight: 700;">
					<view style="display: flex;align-items: center;">
						<image src="/static/new/logo.png" style="width: 40rpx;height: 40rpx;" mode="heightFix"></image>
				...
				<view class="mask_content" ref="noticeContent">
					<view class="contentTip-title" style="color: #333;text-align: left;" v-if="noticeInfo.name">{{noticeInfo.name}}</view>
					<view class="contentTip-box">
						...
						<view class="contentTip-content" v-html="noticeInfo.content"></view>
					</view>
					<view class="btn on" @tap="noticeClick(noticeInfo.id)" v-if="noticeInfo.name">
		...
		<u-popup :show="rnShow" mode="center" round="32" @close="rnShow=false">
			<view class="van-dialog van-dialog2">
				...
				<view class="van-dialog__footer" style="background: #F62A2D;box-shadow: 0px 8rpx 20rpx 0px rgba(246,42,45,0.3);border-radius: 30rpx;">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取弹窗公告
			getNotice() {
				commonScrollApi().then(res => {
					// 如果没有记录或记录的日期不是今天，则显示弹窗
					//  if (!this.oldday || this.oldday !== this.today) {
					// // 记录今天的日期
					// localStorage.setItem('oldday', this.today);
					//  }
					this.noticeList = res.data.data.data
					if (!!this.noticeList.length > 0) {
						this.getNoticeInfo(this.noticeList[0].id)
			...
			noticeClick() {
				...
				if (this.noticeList.length > this.noticeIndex) {
					this.getNoticeInfo(this.noticeList[this.noticeIndex].id)
					this.noticeShow = true;
				}
				// getNoticeInfo(item.id)
				// if (!!this.isCheck) {
				// 	uni.setStorageSync('indexTime', new Date().getTime())
				// } else {
				// 	uni.removeStorageSync('indexTime')
				// }
...
<style lang="scss" scoped>
	...
	.tab1 {
		...
		.item {
			...
			position: relative;
			image,
			img { display: block; border-radius: 20rpx 20rpx 0 0; background: #eee;
				width: 100%;
				height: 220rpx;
			}
			.kl {
	...
	.notice_mask {
		...
		.mask_content {
			...
			.contentTip-title { padding: 30rpx;
				font-size: 40rpx;
				font-weight: 700;
				line-height: 48rpx;
				text-align: center;
			...
			.contentTip-time{ margin-bottom: 30rpx; }
			.contentTip-box{ padding:0 30rpx 30rpx; max-height: 70vh; overflow-y: auto; }
			.contentTip-content {
