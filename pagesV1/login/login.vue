	...
	export default {
		data() {
			return {
				isPageShow: false,
				settingInfo: {},
				baseUrl: 'https://api.wodehongqi.cc',
				lists: [{
						url: 'https://api.wodehongqi.cc',
					},
					{
						url: 'https://api.wodehongqi.cc',
					}
		...
		methods: {
			...
			signalChange(e) {
				// console.log("eee",e.detail.value[0])
			...
			pickerConfirm() {
				// console.log("eee",this.signalindex[0])
			...
			async check_url(url) {
				// console.log('url:' + url)
				await uni.request({
					url: url + "/api/check_level", //仅为示例，并非真实接口地址。
					method: 'GET',
					timeout: 3000,
					success: (res) => {
			...
			async wayGet() {
				for (var i = 0; i < this.lists.length; i++) {
					console.log(this.lists[i].url)
					let [error, res] = await uni.request({
						url: this.lists[i].url + "/api/check_level",
						timeout: 3000,
					});
					if (res && res.data.code === 1) {
			...
			remember(n) {
				...
				console.log('change', this.checkList.checked);
			}
		}
	}
</script>
