<template>
	<view class="container" style="background:#c8131e url(/static/new/bg.jpg) no-repeat center top / 100%;">
		<headerView>
			...
			<view>提款中心</view>
			<template #rightIcon>
				<view class="icon" @click="$utils.toPage('/pages/mine/withdraw/history')">
		...
		<view class="content">
			<view class="main">
				...
				<radio-group @change="wallIDChange" class="cardList">
					<label class="li flex_between" :class="payIndex==index?'on':''" v-for="(item,index) in payList" :key="index">
						<view class="name">
							<image class="icon" :src="item.type==2?require('@/static/mine/alipay2.png'):require('@/static/mine/bank2.png')" mode="widthFix" :class="item.type==2?'alipay':'bank'"></image>
							<image class="icon2" :src="item.type==2?require('@/static/mine/alipay2.png'):require('@/static/mine/bank2.png')" mode="widthFix" :class="item.type==2?'alipay':'bank'"></image>
							{{item.type_str}}</view>
						<!-- <view class="num">{{item.cardno}}</view> -->
						<view class="radio"><radio :value="String(item.id)" :checked="wallID == item.id" activeBackgroundColor="rgba(0, 0, 0, 1)" style="transform:scale(0.7)" /></view>
					</label>
				</radio-group>
				</view>
				...
				<radio-group @change="moneyChange" class="moneyList">
					<label class="li" :class="moneyIndex==index?'on':''" v-for="(item,index) in moneyList" :key="index">
						<view class="img"><image src="/static/new/money.png" mode="widthFix"></image></view>
						<view class="name">{{item.name}}</view>
						<view class="num">{{item.price}}</view>
						<view class="radio"><radio :value="String(item.id)" :checked="moneyIndex == index" activeBackgroundColor="rgba(0, 0, 0, 1)" style="transform:scale(0.7)" /></view>
					</label>
				</radio-group>
				</view>
				<u-skeleton :loading="true" :animate="true" :rows="3" :title="false" v-else></u-skeleton>
				...
				<view class="form">
					<view class="item flex_between">
						<view class="item-input flex_between">
							<input type="number" v-model="price" class="input" placeholder="请输或选择提款金额" placeholder-class="placeholder" />
						</view>
					</view>
					<!-- <view class="ul">
				...
				<view class="form">
					<view class="item flex_between">
						<view class="item-input flex_between">
							<input type="password" v-model="security_password" class="input" placeholder="请输或选择提款金额" placeholder-class="placeholder" />
						</view>
					</view>
				</view>
				...
				<view class="footer col_center">
					...
					<view class="footer-tips">无法提现请联系<span @click="toLoads(settingInfo.service.service_url)">在线客服</span></view>
				</view>
			</view>
				<view class="m-tips" v-if="moneyType=='state'">
					<view class="text">
						<!-- <view class="h2">温馨提示</view> -->
						<view class="desc" v-html="settingInfo.text.withdraw_pension_note"></view>
					</view>
				</view>
				<view class="m-tips" v-else>
					<view class="text">
						<!-- <view class="h2">温馨提示</view> -->
						<view class="desc" v-html="settingInfo.text.withdraw_note"></view>
					</view>
				</view>
		</view>
		...
		<!-- 支付密码修改提示 -->
		<u-popup :show="isPswShow" mode="center" round="32">
			<view class="van-dialog van-dialog2">
				<view class="van-dialog__header">提示</view>
...
<script>
	import {
		...
		withdrawalApi,
		houseWiththreeApi,
		houseWithfourJiaonaApi,
		houseWithfiveJiaonaApi,
		pensionWithdrawWalletApi //个人养老金-可提现钱包
	} from '@/api/api.js'
	export default {
		data() {
			return {
		...
		methods: {
			getMoneyType(){
				if( this.moneyType == 'state'){
					pensionWithdrawWalletApi().then(res => {
						this.moneyList = res.data.data
					})
				} else {
					moneyTypeApi().then(res => {
						this.moneyList = res.data.data
			...
			withdrawal(text) {
				this.$utils.throttle(() => {
					if (this.payList.length == 0) {
						uni.$u.toast('请先绑定银行卡');
					} else if (this.withthreeInfo.status == 3 && this.moneyTypes.includes(this.moneyList[this.moneyIndex].id)) {
						if(!this.price){
						...
						let form = {
							money_type: this.moneyList[this.moneyIndex].id,
							wallet_id: this.payList[this.payIndex].id,
							price: this.price,
						...
						let form = {
							...
							wallet_id: this.wallID,
							price: this.price,
							security_password: this.security_password
						}
						// console.log("form2",form)
						withdrawalApi(form).then(res => {
							...
							if( this.moneyType == 'state'){
								...
								setTimeout(() => {
									uni.redirectTo({
										url: "/pages/mine/withdraw/history"
									})
								}, 1000)								
							}
						})
...
<style lang="scss" scoped>
	.container { min-height: 100vh; background-size: cover;  height: auto; }
	.container { 
		.content { padding:0 30rpx 30rpx; border-radius: 20rpx;
			.main{ background: #fff; padding: 30rpx; border-radius: 20rpx; margin-bottom: 30rpx; }
			...
			.ul {  margin-bottom: 60rpx;  display: flex; justify-content: space-between; gap: 30rpx; flex-wrap: wrap;
				.li { flex: 1; min-width: 190rpx; text-align: center; padding: 30rpx 20rpx; border-radius: 20rpx;  background: linear-gradient(to left, rgba(247, 247, 247, 1), rgba(216, 216, 216, .5)); font-weight: 500; color: rgba(17, 24, 38, 1);
			...
			.cardno{ margin-bottom: 60rpx; 
				.cardList{ display: flex; gap: 30rpx; justify-content: center; align-items: center; }
				.li{ background: rgba(243, 240, 241, 1); padding: 30rpx; border-radius: 20rpx; font-size: .8em; min-width: 200rpx; flex: 1;
					.name{ 
						image{ width: 36rpx; height: 36rpx; vertical-align: middle; margin-right: 10rpx;}
						.icon{ display: none;}
			...
			.moneyList{ flex: 1;
				.li{ display: flex; align-items: center; margin-bottom: 0; background: none; text-align: left; padding: 0;
					.img{
						image{ width: 100rpx; display: block;}
					}
					.name{ width: 260rpx;}
					.num{ flex: 1; color: rgba(246, 42, 45, 1);}
					.radio{}
				}
				.li.on{ background: none; color: #000;}
			}
			.form { margin-bottom: 30rpx;
				.item { margin-bottom: 30rpx;
					.item-input { width: 100%; padding: 26rpx 32rpx; border-radius: 20rpx; background: linear-gradient(to left, rgba(247, 247, 247, 1), rgba(216, 216, 216, .5));
						.input {
							flex: 1;
							font-weight: 500;
							line-height: 48rpx;
	...
	.m-tips{ background: #fff;}
	::v-deep .header { color: #fff;
		.title{ text-align: center; }
	}
	::v-deep .uni-page-head{ display: none !important; }
</style>