<template>
	<view class="container" style="background:#c8131e url(/static/new/bg.jpg) no-repeat center top / 100%;">
		<headerView>
			...
			<view>充值中心</view>
			<template #rightIcon>
				<view class="icon" @click="$utils.toPage('/pages/mine/recharge/history')">
		...
		<view class="content">
			<view class="form">
				...
				<view class="form-nav">
					<view class="item" :class="navIndex == index?'on':''" v-for="(item,index) in navList" @click="onNav(index)">{{item.name}}</view>
				</view>
				<view class="ul" v-if="payList.length > 0">
				<radio-group @change="wallIDChange" class="cardList">
					<label class="li" :class="payIndex==index?'on':''" v-for="(item,index) in payList" :key="index" @click="payIndex=index">
						<view class="name">{{item.pay_type_name}}</view>
				...
				<u-skeleton :loading="true" :animate="true" :rows="3" :title="false" v-else></u-skeleton>
					...
					<view class="tit">
						<view class="item-name">账户余额 <text>￥53.08</text></view>
					</view>
	...
	export default {
		...
		onLoad() {
			...
			getSettingApi().then(res => {
				...
				// this.priceList = res.data.data.recharge_money
				// this.price = res.data.data.recharge_money[0].id
			})
		...
		methods: {
			...
			recharge() {
				rechargeApi({
					money: this.price,
					channel_id: this.payList[this.payIndex].id,
					payment_id: '',
					pay_image: '',
...
<style lang="scss" scoped>
	...
	.container { 
		.content { padding:0 30rpx 30rpx;
			.form-nav{ display: flex; border-radius: 20rpx; overflow: hidden; margin-bottom: 30rpx; 
				.item{ background: rgba(243, 240, 241, 1); flex: 1; padding: 10rpx 20rpx; text-align: center; }
				.item.on{ background: rgba(246, 42, 45, 1); color: #fff; }
			}
			.ul { background: #fff; margin-bottom: 30rpx; display: flex; justify-content: space-between; gap: 30rpx; flex-wrap: wrap;
				.li { min-width: 190rpx; flex: 1; text-align: center;
					padding: 30rpx 20rpx;
					border-radius: 20rpx;
					background: linear-gradient(to left, rgba(247, 247, 247, 1), rgba(216, 216, 216, .5));
					font-weight: 500;
					color: rgba(17, 24, 38, 1);
					.name{ font-weight: 300; font-size: .8em; margin-bottom: 10rpx; }
					.num{ line-height: 1; }
					...
					.li-box {
						...
						height: 36rpx;
						border-radius: 36rpx;
						border: 2rpx solid rgba(177, 177, 177, 1); background: #fff;
					}
			...
			.form { background: #fff; padding: 30rpx; border-radius: 20rpx; margin-bottom: 30rpx;
				.tit{ display: flex;
					.item-name { flex: 1;
						margin-bottom: 16rpx;
						font-weight: bold; 
				...
				.item { 
					.item-input { margin-bottom: 30rpx;
						width: 100%;
						padding: 30rpx 40rpx;
						border-radius: 16rpx;
						border: 2rpx solid rgba(232, 235, 238, 1); background: rgba(232, 235, 238, .5);
						...
						.placeholder {
							color: rgba(187, 189, 193, 1); font-weight: 400; 
						}
					}
