...
{{lotteryInfo.desc}}</view>
		...
		<view class="m-tips">
			<view class="text">
				...
				<view class="desc">直推1人:赠1次抽奖 &+提升好运值</view>
			</view>
		</view>
		<view class="f-desc">温馨提示:坚持签到抽奖更幸运</view>
	
	<u-popup :show="dialerShow" mode="center" round="32" :closeable="true" @close="dialerShow=false">
		<view class="van-dialog van-dialog2">
			...
			<view class="van-dialog__footer">
				<view class="button" @click="dialerShow=false">关闭</view>
			</view>
		</view>
	</u-popup>
	</view>
...
<script>
	import { lotteryAPI,lotteryInfoAPI,lotteryRaffleAPI } from '@/api/api.js'
	export default {
		data() {
			return {
		...
		methods: {
			...
			getRaffle(){
				lotteryRaffleAPI({ids:JSON.stringify(this.idList)}).then(res => {
...
<style lang="scss">
	...
	.title{ text-align: center; background: rgba(0, 0, 0, 0.4);padding: 20rpx 60rpx;border-radius: 90rpx; color: #fff; margin: 60rpx auto 90rpx;}
	.m-dialer{  margin: 0 auto 90rpx; }
	::v-deep .l-dialer__inner{
		.l-dialer__inner-border{transform: scale(1.11);}
		.l-dialer__inner-name{width: 4em; line-height: 1; text-align: center;}
		// .l-dialer__inner-item{ background: none !important; }
	}
	.info{ background: rgba(0, 0, 0, 0.4); color: #fff; padding: 20rpx 30rpx; border-radius: 20rpx; margin: 0 30rpx 30rpx;}
	.redbag{ background: #fff; padding: 20rpx 30rpx; border-radius: 20rpx; margin: 0 30rpx 30rpx;
		.item{ display: flex; align-items: center; padding: 20rpx 0;
			image{ width: 60rpx; height: 60rpx; display: block;}
			.text{padding-left: 20rpx;
				text{ color: rgba(243, 62, 49, 1); }
			}
		}
	}
	.m-tips{ background: #fff; padding: 20rpx 30rpx; border-radius: 20rpx; margin: 0 30rpx 30rpx;}
	.f-desc{ color: #fff;  margin: 0 30rpx 90rpx; font-size: .9em;}
	.scroll { background: rgba(0, 0, 0, 0.4); color: #fff; padding: 30rpx; border-radius: 20rpx; margin: 0 30rpx 30rpx; flex: none; 
		.scroll-box{ overflow: hidden; height: 80rpx;}
		.scroll-content { display: flex; flex-direction: column; animation: verticalScroll linear infinite;
			.item { background: rgba(0, 0, 0, 0.0); padding: 10rpx 0; height: 60rpx; line-height: 40rpx;  border-radius: 20rpx; white-space: nowrap; font-size: .9em; }
		}
