<template>
	<view class="container" style="background:#c8131e url(/static/new/bg.jpg) no-repeat center top / 100%;">
		<headerView>
			...
			<view>转账中心</view>
			<template #rightIcon>
				<view class="icon" @click="$utils.toPage('/pages/mine/transfer/history')">
			...
			<view class="main">
				...
				<radio-group @change="wallIDChange" class="moneyList">
					<label class="li" :class="form.paytype==item.id?'on':''" v-for="(item,index) in priceList" :key="index">
						...
						<view class="radio"><radio :value="String(item.id)" :checked="form.paytype == item.id" activeBackgroundColor="rgba(0, 0, 0, 1)" style="transform:scale(0.7)" /></view>
					</label>
				</radio-group>
				</view>
				...
				<view class="form">
					<view class="item">
						...
						<view class="item-input flex_between">
							<input type="number" v-model="form.price" class="input" placeholder="输入互转金额"
					...
					<view class="item">
						...
						<view class="item-input flex_between">
							<input type="text" v-model="form.name" class="input" placeholder="输入对方姓名"
								placeholder-class="placeholder" />
						</view>
					</view>
					<view class="item">
						...
						<view class="item-input flex_between">
							<input type="number" v-model="form.phone" class="input" placeholder="输入对方手机号"
					...
					<view class="item">
						...
						<view class="item-input flex_between">
							<input type="password" v-if="pwdShow" v-model="form.security_password" class="input"
								placeholder="输入提款密码" placeholder-class="placeholder" />
							<input type="text" v-else v-model="form.security_password" class="input" placeholder="输入提款密码"
								placeholder-class="placeholder" />
					...
					<view class="footer col_center">
						...
						<view class="footer-tips">无法互转请联系<span @click="toLoads(settingInfo.service.service_url)">在线客服</span></view>
					</view>
				</view>
			</view>
				<view class="m-tips">
			...
			transfer() {
				...
				this.$utils.throttle(() => {
					transferApi(this.form).then(res => {
						...
						setTimeout(() => {
							uni.redirectTo({
								url: "/pages/mine/transfer/history"
							})
						}, 1000)
					})
				}, 2000)()
	...
	.container {
		.content { padding: 0 30rpx 30rpx;
			...
			.ul {  margin-bottom: 60rpx;  display: flex; justify-content: space-between; gap: 30rpx; flex-wrap: wrap;
				.li { width: 300rpx; flex: none; text-align: center; padding: 40rpx 20rpx; border-radius: 20rpx;  background: linear-gradient(to left, rgba(247, 247, 247, 1), rgba(216, 216, 216, .5)); font-size: .8em; font-weight: 300; color: rgba(17, 24, 38, 1); 
					.name{ font-weight: 300; line-height: 1; margin-bottom: 10rpx; }
					.num{ line-height: 1; font-weight: bold; font-size: 1.4em; }
				}
				.li.on{ background: linear-gradient(to left, rgba(255, 98, 75, 1), rgba(255, 54, 80, 1)); color: #fff; font-weight: 500;  }
			}
			.moneyList{ flex: 1;
				.li{ width: 100%; display: flex; align-items: center; margin-bottom: 0; background: none; text-align: left; padding: 0;
					.img{
			...
			.form { margin-bottom: 30rpx;
				.item {
					...
					.item-input {
						...
						padding: 26rpx 32rpx;
						border-radius: 20rpx;
						border: 2rpx solid rgba(232, 235, 238, 1); background: rgba(243, 240, 241, 1);
						.input {
						...
						.all{ background: rgba(246, 42, 45, 1); color: #fff; padding: 10rpx 30rpx; border-radius: 90rpx;}
						.placeholder {
							color: rgba(187, 189, 193, 1);
						}
					}
					.item-input:focus,.item-input:hover{ background: #fff; }
