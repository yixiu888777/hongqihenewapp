	...
	<view class="container" style="background:#c8131e url(/static/new/bg.jpg) no-repeat center top / 100%;" v-if="isPageShow">
		...
		<view class="main">
				...
				<view class="list" v-if="list.length > 0">
					<view class="item" v-for="item in list" :key="item.id">
						...
						<view class="flex_between">
							...
							<view class="icon">
								<view @click.stop="$utils.toPage(`/pages/mine/address/add?id=${item.id}`)"><image src="/static/new/icon-edit.png" mode="heightFix"></image></view>
			...
			<view class="footer flex_center">
				<view class="button" @click="$utils.toPage('/pages/mine/address/add')">添加</view>
			</view>
		</view>
...
<style lang="scss">
	.container {
		...
		.content { margin-bottom: 30rpx;
			.list { 
				.item { background-color: rgba(255, 255, 255, 1); border-radius: 20rpx;
					margin-bottom: 30rpx;
					padding: 30rpx;
					border-radius: 20rpx;
					...
					.icon{
						image{ height: 30rpx; display: block; }
					}
					.choose { 
