<template>
	<view v-if="show" class="tt-sign-board-class tt-sign-board" :style="{height: `calc(100% - ${customBarHeight}px)`}">
		<!-- 签名canvas -->
		<view class="tt-sign-board__content">
			<view class="tt-sign-board__content__wrapper">
				<canvas class="tt-sign-board__content__canvas" :canvas-id="canvasName" :disableScroll="true"
					@touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd"></canvas>
			</view>
		</view>

		<!-- 底部工具栏 -->
		<view class="tt-sign-board__tools">
			<!-- 可选颜色 -->

			<!-- 按钮 -->
			<view class="tt-sign-board__tools__button">
				<view class="tt-sign-board__tools__button__item tt-bg-red" @tap="reDraw">清除</view>
				<view class="tt-sign-board__tools__button__item tt-bg-blue" @tap="save">保存</view>
			</view>
		</view>

		<!-- 伪全屏生成旋转图片canvas容器，不在页面上展示 -->
		<view style="position: fixed; left: -2000px;width: 0;height: 0;overflow: hidden;">
			<canvas canvas-id="temp-tt-sign-canvas"
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取画板的相关信息
			getCanvasInfo() {
				this._tGetRect('.tt-sign-board__content__canvas').then(res => {
			...
			async getRotateImage(dataUrl) {
				...
				// 创建新画布
				// const tempCtx = uni.createCanvasContext('temp-tt-sign-canvas', this)
